...
export function CaseWizard({ caseId }: { caseId: string }) {
  ...
  const hydrate = (data: CostingCaseAggregate) => {
    ...
    const step2NotYetSaved = data.costingCase.currentStep <= 2;

    setCosts(data.costs.length ? data.costs : step2NotYetSaved ? [
      {
    ...
    setIncome(data.income.length ? data.income : step2NotYetSaved ? [
      {
